...
from alphafold.model import data, utils
...
flags = tf1.app.flags
flags.DEFINE_string('outprefix', '', help='string prefix for all outfiles')
...
flags.DEFINE_integer('crop_size', 11, help='Max size of training example; set this '
                     'as low as possible for memory and speed')
flags.DEFINE_integer('num_epochs', 200, help='number of epochs')
...
flags.DEFINE_integer('print_steps', 100,
...
flags.DEFINE_bool('freeze_binder', True, help='if True, dont fit binder params')
...
flags.DEFINE_float('pae_binder_slope', -7.9019634017508675, 
                   help='initial slope for PAE binder model')
flags.DEFINE_float('plddt_binder_slope', 16.690735, 
                   help='initial slope for pLDDT binder model')
flags.DEFINE_multi_float('binder_intercepts', [0.80367635, 0.43373787], required=False,
...
def get_loss_fn(model_params, key, processed_feature_dict, structure_flag):
    ...
    loss = sf*(predicted_dict['structure_module']['fape'])
...
def create_batch_from_dataset_row(row):
    ...
    batch['native_exists'] = row.native_Pnear
    #print(111111111111111111)
    #print(batch['native_exists'])
    return batch
...
def compute_valid_stats(valid_loader, replicated_params, jax_key):
    ...
    for n, batch in enumerate(valid_loader):
        ...
        print(
               'valid_epoch_n=', e, n,'loss=', np.mean(predicted_dict['structure_module']['fape']),

              )


        temp_train_loss.append(np.mean(predicted_dict['structure_module']['fape']))
        ...
        print(f'each_valid_set_mean_loss: {global_step} {n}, loss: {mean_loss}', flush=True)
        if (n+1)%FLAGS.valid_steps==0:
           print(f'Valid_epoch_loss: {global_step} {n}, loss: {mean_loss}', flush=True)
        sys.stdout.flush()
...
for e in range(num_epochs):
    ...
    for n, batch in enumerate(train_loader):
        ...
        if (grads_sum_count >= FLAGS.apply_every and 
        ...
        if (n+1) % FLAGS.print_steps == 0:
            ...
            sidechain_fape = round(float(np.mean(temp_sidechain_fape)),4)
            chi_loss = round(float(np.mean(temp_chi_loss)),4)
            print(f'Train_epoch_loss: {global_step}, loss: {mean_loss},', flush=True)
        ...
        if (n+1) % FLAGS.save_steps == 0:
            ...
            step_fname = f'/data/lab/czg/HighFold-MeD/finetune_params/{prefix}alphafold_cyclicl_step.npy'
            np.save(step_fname, global_step)
            param_fname = f'/data/lab/czg/HighFold-MeD/finetune_params/{prefix}alphafold_cyclic_params_{global_step}.pkl'
            save_params = jax.tree_map(lambda x: x[0,], replicated_params)
            with open(param_fname, 'wb') as f:
                pickle.dump(save_params, f)
            state_fname = f'/data/lab/czg/HighFold-MeD/finetune_params/{prefix}alphafold_cyclic_state_{global_step}.pkl'
            with open(state_fname, 'wb') as f:
                pickle.dump(opt_state, f)
            config_fname = f'/data/lab/czg/HighFold-MeD/finetune_params/{prefix}alphafold_cyclic_config.pkl'
